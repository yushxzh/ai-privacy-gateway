...
test('mitmproxy 压缩正文经过真实本地检查入口，认证与工具结构保留，凭据不确认外发', async t => {
  ...
  assert.deepEqual(records.counts(), { total: 22, masked: 2, blocked: 1, allowed: 0, unchecked: 19 })
  assert.equal(records.summaries().filter(record => record.inspectionIssue === 'unsupported').length, 18)
