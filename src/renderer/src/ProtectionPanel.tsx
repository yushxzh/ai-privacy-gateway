...
const labels = { off: '未开启', starting: '正在接入', configured: '已配置 · 等待请求', verified: '已验证', stopping: '正在恢复', removing: '正在移除', error: '需要处理' }
export function ProtectionPanel({ value, busy, change }: {
  ...
  const pending = busy || value.state === 'starting' || value.state === 'stopping' || value.state === 'removing'
  ...
  return <section className="panel native-connection" aria-label="WorkBuddy 一键接入">
    <div className="native-connection-heading">
      ...
      <div className="native-connection-title"><h2>WorkBuddy</h2><p>沿用原登录和模型 · 文本请求</p></div>
    ...
    <div className="native-connection-body">
      ...
      <small>检查模型请求中的文本。独立上传、其他端点及工具直接联网不在覆盖范围；未适配的模型格式会停止外发。</small>
