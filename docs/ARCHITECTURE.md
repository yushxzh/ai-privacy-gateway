...
0.1.6 的 WorkBuddy 原生连接由桌面接入协调器管理。开启和恢复的顺序如下：
...
接入操作串行执行。客户端未退出时不强杀，恢复失败保留代理与备份；代理意外结束或外部修改代理配置时，撤销「已验证」。TLS 子进程在检查入口连续失联后自行退出。强制退出网关不能保证即时恢复 WorkBuddy，下一次启动根据备份尝试恢复连接。
